...
class UserAdapterSec(
    val list: ArrayList<UserDataSec>,
    val OnItemClick: (UserDataSec) -> Unit
) : RecyclerView.Adapter<UserAdapterSec.VH>() {

    inner class VH(val itemRecycBinding: ItemRecycAllBinding) :
        RecyclerView.ViewHolder(itemRecycBinding.root) {
        fun onBind(data: UserDataSec, position: Int) {
